...
description: >-
  ...
  导出和文件审计。数学建模场景额外支持原始数据/过程/结果等证据图分类、按需的子问题证据检查和建模流程图规范。
...
并扩展数学建模中的证据图分类、按需的子问题证据检查和建模流程图。**永远先思考再画**：
...
流程图属于方法与结构示意，不替代用于支撑数值结论的结果证据；仓库默认不设置候选图数量配额。
...
- **图型服从信息结构**：避免无理由地全部使用同一种图型，也不要为了“多样”强行换图；每个面板回答唯一问题，不重复展示相同数据（详见 `design_theory.md` §11 反冗余清单）
...
数学建模场景（两种语言均适用）：按论证需要选择原始数据图、过程图和结果图。每个子问题的核心结论必须有足够证据，但不要求每问都生成三类图，也不设置统一最低图数或最低图型种类；图型由数据结构和论证目标决定。
...
# 2. 数学建模 P2 图文件审计：默认只检查实际存在的图；固定类别数量仅在官方/用户明确要求时启用
python "<SKILL_ROOT>/references/roles/编程手/scripts/figure_audit.py" "<PROJECT_ROOT>/figures" --questions q1 q2 ... qN --strict
# 如确有类别数量要求，再追加：--require-category-check --min-per-category N
```

- `check_figure.py`：检查单张图的格式合规性（DPI、字体、尺寸、矢量格式），编程手每次导出后自检用。
- `figure_audit.py`：默认检查已有图的配对格式、DPI、可编辑文本等基础完整性；只有官方规则或用户明确要求时才检查 raw/process/result 类别数量。

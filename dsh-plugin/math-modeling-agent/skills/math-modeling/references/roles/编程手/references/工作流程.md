...
6. 生成建模流程图（`flow_overall_model` / `flow_qN_model`）：按 `tools/figure/references/chart-types/flowchart.md` 执行，常规 1 幅总体图加必要的子问题图，节点与连线来自真实模型/代码，用已安装的 Matplotlib/MATLAB 生成并保留可运行代码；流程图不计入三类候选图配额，也不替代结果图。

...
该检查会报告篇幅、公式/图/表数量、图表编号与正文引用、参考文献双向对应以及页数问题。仓库不再把非官方的最低字数、最低页数、最低公式数、最低图数或最低表数作为默认失败条件；只有当届官方规则或用户明确设定的阈值才作为相应门槛。2026 年 CUMCM 正文 30 页上限属于已经核验的官方硬约束。OOXML 校验通过后仍需渲染抽检，因为 XML 合法不等于版面正确。

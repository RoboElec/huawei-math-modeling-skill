...
| ② | [编程手](references/roles/编程手/SKILL.md) | 编写并运行 Python/MATLAB，生成结果与图 | `P1` 最小可运行结果、`P2` 编程终检 | 代码、结果表格、按证据需要生成的图表、至少 1 幅总体建模流程图、`results/复现清单.json` |
| ③ | [论文手](references/roles/论文手/SKILL.md) | 基于真实结果构建论证并生成 Word 论文 | `W1` 证据大纲、`W2` 论文终检 | 由证据链决定正式图表数量；默认交付 `完整论文.docx`；用户显式要求时同时交付 LaTeX 源码项目、PDF 与哈希清单 |

质检 Subagent 是阶段内只读验收者，不是第四个固定角色。默认只启用固定质检；每个固定门禁最多两轮（discovery + regression），第二轮仍失败则停止自动返工并交由人工决策。其他协作仅在用户明确选择后运行。完整协议见 [Subagent 调度与阶段门禁](references/Subagent调度.md)。
...
- 不设置跨竞赛统一的最低字数、最低页数或最低图数。非官方质量目标只作为提示，真正的硬门槛来自当届官方规则、数学正确性、复现与证据一致性。

...
`P1` 第一次失败后修复已编号阻塞项并最多回归复验一次。第二次仍失败时停止自动返工；只有用户显式 `HUMAN_OVERRIDE:P1` 后才继续全量计算，并保留风险记录。P1 不要求固定图数、完整敏感性分析或最终性能。
...
3. 证据驱动图表：按需要使用 `raw_q1_*`、`process_q1_*`、`result_q1_*` 命名。每个子问题的核心结论必须有可追溯证据，但不要求每问具备三类图或统一最低图数；生成的正式图同时导出 SVG 与 300 DPI PNG。
4. 将 `ROLE_ROOT/scripts/plot_style.py` 或 MATLAB 出版绘图工具复制到 `PROJECT_ROOT/utils/` 后使用。
5. 所有正式图经 `tools/figure/scripts/export_figure.py` 导出后，运行 `figure_audit.py ... --questions q1 q2 ... qN --strict` 检查已有图；仅在官方/用户明确要求类别数量时增加 `--require-category-check --min-per-category N`。实际打开 PNG 进行视觉检查；发现阻塞性错误回到源代码修正。
...
代码、结果表、支撑各子问题核心结论的证据和复现清单完成作者自检并冻结后，按 `../../../../references/Subagent调度.md` 执行 P2。质检者重跑唯一复现命令，核对哈希、环境、种子、参数、关键数值、约束、边界、量纲、统计计算和图表语义；`figure_audit.py --strict` 与实际成图作为证据输入。P2 最多两轮；第二次仍失败时停止自动返工，只有用户显式 `HUMAN_OVERRIDE:P2` 后才进入论文阶段。

...
4. 区分“硬错误/阻塞项”和“质量建议”。数学正确性、当届官方硬约束、关键复现失败、数据/结果矛盾、缺失必需文件属于阻塞项；非官方的篇幅、图表数量、公式数量、额外实验、美化和措辞等质量目标默认只记为非阻塞建议。只有官方规则或用户明确设定的质量门槛才能升级为阻塞项。
5. 环境缺少引擎、搜索源、渲染器或依赖时，报告阻塞并继续完成仍可验证的部分；禁止静默换工具、跳过验证或用较差产物冒充完成。
6. 运行环境支持 Subagent 且任务会生成或修改固定交付物时，按 `references/Subagent调度.md` 在关键节点派发独立质检；真实竞赛默认执行“每门禁最多 2 次：一次全量审查 + 一次回归复验”的预算与人工熔断规则，禁止无限返工。作者自检不能替代独立验收。除固定质检外，不主动派发其他 Subagent；仅在用户明确启用具体协作任务时按所选范围执行。
7. 交付前运行当前角色规定的全部确定性门禁。存在未解决的阻塞性 `P0/P1` 时不得把状态声称为 `PASS`。若某门禁已耗尽两轮复验预算，用户可以显式使用 `HUMAN_OVERRIDE` 接受风险并继续；此时必须保留原 FAIL 证据并标记为“人工接受风险/受限交付”，不得伪装成独立验收通过。
...
- `FAIL` 必须按证据返回对应角色修正，但每个门禁默认最多进行一次全量审查和一次回归复验；预算耗尽后停止自动返工并交给用户决策。只有修改实际触及该门禁合同或证据时，相关 `PASS` 才失效。
...
3. 按论证需要生成原始数据图、模型运行过程图和模型最终结果图。每个子问题必须有足以支持核心结论的证据，但证据可以是图、表、关键数值、解析推导或可复现输出；不强制每个子问题同时具备三类图，也不设置统一最低图数。生成图时使用 `raw_q1_*`、`process_q1_*`、`result_q1_*` 等命名；相同内容的 SVG、PNG 和灰度预览按 1 张逻辑图计数。
...
论文不设置跨竞赛统一的最低图数。正式图表数量由题目结构、证据链和当届官方规则决定；不为凑数量制造与结论无关的图。官方或用户明确规定图数时再按其要求执行。

## 阶段反馈闭环

- 任一 Subagent 门禁返回 `FAIL` 时，按回执证据回到对应角色；修正后只重跑受影响门禁，并受 `references/Subagent调度.md` 的两轮复验预算约束。第二次仍失败时停止自动返工，保留当前最佳版本并交由用户决策，不得由主 Agent 口头覆盖。
...
3. 模型复杂度由问题需要决定。简单模型若与题目结构、数据规模和评价目标匹配，可以作为主模型或基线；创新应来自问题结构、数据处理、约束设计、算法改进或验证方式，不以无必要地堆叠模型作为创新。
...
- 论文已按目标竞赛当届官方规则配置，全部子问题都有可追溯证据，实际存在的图表编号与正文引用连续，参考文献与正文引用双向对应。非官方的最低字数、公式数、图数、表数仅作为可选质量提示，不得单独导致门禁失败。Word 分支还须通过原生 OMML、DOCX 结构和渲染检查；LaTeX 分支还须通过环境诊断、真实编译、权威资源—源码—PDF 哈希绑定、正文与附录页数、空白页、页面尺寸、字体嵌入和图片 DPI 检查。
